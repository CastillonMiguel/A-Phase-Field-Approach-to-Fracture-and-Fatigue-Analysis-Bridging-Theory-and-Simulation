r"""
.. _ref_example_geo_specimen_3_H08:

Specimen 3
----------
This example demonstrates the procedure for generating a mesh for the compact tension specimen presented in :footcite:t:`example_Wagner2018_phd_thesis`, from which some of the simulation results will be compared. In this case, the mesh corresponds to an initial crack position of $H=0.8$ mm following the coordinate system presented in the .geo file.

The parameter $H$ represents the vertical position of the initial crack tip measured from the specimen centerline. This specific configuration ($H=0.8$ mm) represents one of the test cases used to study crack path deviation in specimens with multiple holes, where the asymmetric crack position leads to complex crack propagation behavior.
...
.. include::  ../../../../examples/GmshGeoFiles/Compact_specimen/specimen_3_H08.geo
   :literal:
...
# %%
# Open the .geo file
geo_file = os.path.join(folder, "specimen_3_H08.geo")
gmsh.open(geo_file)
...
if save_image:
   ...
   # Save the screenshot
   image_file = os.path.join(folder, "specimen_3_H08.png")
